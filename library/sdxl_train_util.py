...
def timestep_embedding(timesteps, dim, max_period=10000, dtype=torch.float32):
    ...
    """
    half = dim // 2
    freqs = torch.exp(-math.log(max_period) * torch.arange(start=0, end=half, dtype=dtype) / half).to(
...
def get_timestep_embedding(x, outdim, dtype=torch.float32):
    assert len(x.shape) == 2
    b, dims = x.shape[0], x.shape[1]
    x = torch.flatten(x)
    emb = timestep_embedding(x, outdim, dtype=dtype)
    emb = torch.reshape(emb, (b, dims * outdim))
    return emb


def get_size_embeddings(orig_size, crop_size, target_size, device, dtype=torch.float32):
    emb1 = get_timestep_embedding(orig_size, 256, dtype=dtype)
    emb2 = get_timestep_embedding(crop_size, 256, dtype=dtype)
    emb3 = get_timestep_embedding(target_size, 256, dtype=dtype)
